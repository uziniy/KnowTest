...
questions_and_answers = {
    ...
    # "시리얼 통신에서 사용하는 기본 통신 속도 단위는 무엇인가요?": "bps",
    ...
    # "플로팅(floating) 상태를 방지하기 위해 사용하는 저항의 종류는 무엇인가요?": "풀업 풀다운",
    ...
    # "I2C 프로토콜에서 사용되는 주소의 비트 길이는 몇 비트인가요?": "7",  
    # "SDA 라인은 양방향 통신이 가능하다.(o,x)": "o",
    # "SCL은 마스터의 클럭 신호를 전달하는 라인으로 데이터 전송의 타이밍을 동기화하는 역할을 한다.(o,x)": "o",
    # "I2C 프로토콜은 반이중 통신을 한다.(o,x)": "o",
    # "마스터 혹은 슬레이브로 I2C 통신을 시작하는 함수는?": "Wire.being()",
    # "I2C에서 address로 데이터 전송을 시작하는 함수는?": "Wire.beginTransmission()",
    # "I2C에서 value를 데이터 버퍼에 기록하는 함수는?": "Wire.write()",
    # "I2C에서 쓰기를 마치면서 데이터 버퍼의 모든 값들을 실제로 전송하는 함수는?": "Wire.endTransmission()",
    # "마스터로부터 데이터가 전송될 때 호출될 함수 func를 등록하는 함수는?": "Wire.onReceive()",
